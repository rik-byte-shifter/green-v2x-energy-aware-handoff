...
\begin{itemize}
\item Creates the baseline highway scenario via \texttt{SimulationConfig.sparse\_demonstration\_scenario()} (now configured for \textbf{overlapping coverage}: 16 BSs, 400 m radius, heavy-rain profile with 10 dB shadowing).
...
\item Also reports \textbf{Bangladesh-grid CO\textsubscript{2} sensitivity} using \texttt{SimulationConfig.bangladesh\_grid\_scenario()}.
\item Optional standalone weather study: \texttt{python weather\_sweep.py} (cross-weather uncertainty with 95\% CI).
...
\item Practical runtime: this full suite is compute-heavy and typically takes \textbf{about 2--3 hours} on a laptop-class CPU.
\end{itemize}
...
\item Prints regional CO\textsubscript{2} sensitivity snapshots (Bangladesh-intensity scenario) for manuscript discussion.
\end{itemize}
...
\item Outage decomposition terms:
\item \texttt{outage\_probability\_percent} (total outage),
\item \texttt{coverage\_gap\_percent} (no serving BS / no coverage),
\item \texttt{sinr\_outage\_percent} (connected but SINR below threshold).
\end{itemize}
...
\subsection{5. Multi-weather uncertainty sweep: \texttt{weather\_sweep.py}}
\begin{itemize}
\item Standalone script to evaluate robustness across all defined weather profiles in \texttt{src/models/weather.py}.
\item Uses default multi-seed set \texttt{[42,123,456,789,1011]} and computes mean + 95\% confidence intervals (t-distribution) per weather.
\item Core outputs:
\item \texttt{weather\_sweep.json}
\item \texttt{weather\_sweep\_energy\_saving.png}
\item \texttt{weather\_sweep\_handoff\_reduction.png}
\item \texttt{weather\_sweep\_co2\_saving.png}
\item Supports CLI overrides for duration, time-step, number of vehicles, base stations, and weather subset for reproducible ablations.
\end{itemize}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{results/weather_sweep_energy_saving.png}
\caption{Weather-sweep mean energy saving vs RSSI with uncertainty across seeds.}
\label{fig:weather_sweep_energy_saving}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{results/weather_sweep_handoff_reduction.png}
\caption{Weather-sweep handoff reduction vs RSSI across weather profiles.}
\label{fig:weather_sweep_handoff_reduction}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{results/weather_sweep_co2_saving.png}
\caption{Weather-sweep CO\textsubscript{2} saving vs RSSI across weather profiles.}
\label{fig:weather_sweep_co2_saving}
\end{figure}

\subsection{6. Recent codebase fixes reflected in current results}
\begin{itemize}
\item \textbf{State reset fairness}: \texttt{V2XSimulator.run\_algorithm()} uses explicit network-state reset so each algorithm starts from clean BS load state.
\item \textbf{Outage decomposition}: total outage is split into \texttt{coverage\_gap\_percent} and \texttt{sinr\_outage\_percent}, reducing ambiguity in failure attribution.
\item \textbf{Scenario geometry correction}: \texttt{sparse\_demonstration\_scenario()} now uses \texttt{num\_base\_stations=16}, \texttt{bs\_coverage\_radius=400}, and \texttt{shadowing\_std\_db=10.0} so vehicles see overlapping candidate BSs and baseline algorithms remain differentiable.
\item \textbf{Scaling scenario correction}: \texttt{run\_scaling\_experiment()} uses \texttt{SimulationConfig.scaling\_scenario()} to avoid high-load coverage collapse.
\item \textbf{Capacity realism fix}: \texttt{BSConfig.max\_capacity} default is increased from 20 to 100, preventing artificial association blocking in 200-vehicle scalability runs.
\item \textbf{Hardware-validation scope}: \texttt{EnergyModelValidator.validation\_passed} is based on in-scope OBU-side anchors; BS anchors are retained as contextual rows.
\end{itemize}
...
\item \textbf{Hardware validator compatibility fields}:
\item Output preserves compatibility keys such as \texttt{mean\_absolute\_error}, \texttt{max\_deviation\_percent}, and \texttt{mean\_absolute\_error\_obu}.
\end{itemize}
...
\item \textbf{\texttt{docs/literature\_review.py}} --- structured \texttt{LITERATURE\_REFERENCES} (currently $\sim$20+ keys; check \texttt{list\_all\_keys()}), \texttt{generate\_related\_work\_section()}, aligned with BibTeX keys for the final paper.
...
\item \textbf{\texttt{weather\_sweep.py}} --- weather-robustness uncertainty study with 95\% CI plots and JSON summary.
\item \textbf{\texttt{debug\_metrics.py}} --- quick sanity-check script for algorithm-selection differentiation in a minimal two-BS setup.
\item \textbf{\texttt{requirements.txt}} --- reproducible Python dependency baseline for simulation, plotting, and tests.
\end{itemize}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{results/extrapolation_validation.png}
\caption{Multi-duration extrapolation validation for energy, CO\textsubscript{2}, and handoff-rate stability.}
\label{fig:extrapolation_validation}
\end{figure}
